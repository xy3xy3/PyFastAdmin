...
from app.services.task_registry import list_queue_consumers, resolve_display_values
from app.tasks import load_builtin_tasks


@dataclass(frozen=True, slots=True)
class ConsumerTableColumn:
    """队列页面表格列。"""
...
_BASE_COLUMNS = [
    ConsumerTableColumn(key="key", label="消费者 Key"),
    ConsumerTableColumn(key="name", label="消费者名称"),
    ConsumerTableColumn(key="stream", label="Stream"),
    ConsumerTableColumn(key="group", label="消费组"),
    ConsumerTableColumn(key="tags", label="标签"),
    ConsumerTableColumn(key="last_status", label="最近状态"),
    ConsumerTableColumn(key="last_message_id", label="最近消息 ID"),
    ConsumerTableColumn(key="success_count", label="成功次数"),
    ConsumerTableColumn(key="failure_count", label="失败次数"),
    ConsumerTableColumn(key="retry_count", label="重试次数"),
    ConsumerTableColumn(key="dead_letter_count", label="死信次数"),
    ConsumerTableColumn(key="worker_id", label="最近 Worker"),
]
...
async def build_consumer_table_payload(filters: dict[str, str]) -> dict[str, Any]:
    """构建队列消费者页面数据。"""

    load_builtin_tasks()
    definitions = list_queue_consumers()

    dynamic_columns: dict[str, ConsumerTableColumn] = {}
    tag_values: list[str] = []
    rows: list[dict[str, Any]] = []

    for definition in definitions:
        monitor = await task_monitor_service.get_consumer_monitor(definition.key)
        ...
        for column in definition.display_columns:
            if column.key not in dynamic_columns:
                dynamic_columns[column.key] = ConsumerTableColumn(key=column.key, label=column.label)
        ...
        row_values = {
            ...
            "last_message_id": monitor.get("last_message_id", "-"),
            "success_count": monitor.get("success_count", "0"),
            "failure_count": monitor.get("failure_count", "0"),
            "retry_count": monitor.get("retry_count", "0"),
            "dead_letter_count": monitor.get("dead_letter_count", "0"),
